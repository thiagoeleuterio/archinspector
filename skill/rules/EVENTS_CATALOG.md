...
Future Rule IDs in this catalog must use `MSG-NNN`, with sequence numbers scoped to the Events and Messaging category.

The repository contains `skill/rules/events/EVENT-001.md`. `EVENT-001` is a published legacy identifier compatible with the official `MSG` prefix and remains traceable without renaming. Future rules in this category must use the official `MSG` prefix.

This catalog must not create a duplicate `MSG-001` rule for the same responsibility.
...
| --- | --- | --- |
| Event semantics | EVENT-001, MSG-006 | Domain fact meaning and name-to-behavior consistency are separate. |
