...
Evaluate whether exceptions, suppressions, exclusions, baselines, and deviations have justification, ownership, scope, and lifecycle control.
...
Apply this rule when architecture verification uses exceptions, suppressions, exclusions, baselines, waivers, ignored tests, allowed deviations, or equivalent mechanisms.
...
Use Not Applicable when no exception mechanism exists, deviations are not accepted, or the risk is governed outside reviewed material. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether exceptions, suppressions, exclusions, baselines, and deviations have justification, ownership, scope, and lifecycle control, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Are accepted deviations controlled enough not to hide architectural risk indefinitely? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes suppression list, baseline file, exclusion rules, owner, justification, expiry or review cadence, scope, change history, and approval records. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether exceptions, suppressions, exclusions, baselines, and deviations have justification, ownership, scope, and lifecycle control. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: permanent suppression, where expected evidence is suppression metadata and the problem is no review path; broad exclusion, where expected evidence is exclusion scope and the problem is hiding relevant architecture evidence; baseline accepts new violations, where expected evidence is baseline behavior and history and the problem is drift folded into accepted state.

Remediation guidance should add justification, owner, scope, review cadence or expiry where proportionate, narrow broad exclusions, distinguish accepted baseline from new violations, and avoid requiring zero exceptions or universal automation.

Internal boundaries: TEST-006 owns forbidden dependency detection; TEST-017 owns regression after accepted state; TEST-019 owns maintainability of exception mechanisms. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when accepted deviations are justified, scoped, owned, reviewable, and controlled through proportionate lifecycle evidence, without requiring zero exceptions, suppressions, exclusions, or baselines.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and exceptions, suppressions, exclusions, baselines, ignored tests, or deviations are broad, permanent, unjustified, ownerless, unreviewed, or hide new architectural violations.
...
Use Warning when governance exists but justification, owner, scope, expiry, review cadence, or baseline behavior is incomplete or weak without confirmed hidden violations.
...
Use Not Applicable when no exception mechanism exists, deviations are not accepted, or the risk is governed outside reviewed material.
...
Use Not Enough Evidence when suppression lists, baseline files, exclusion rules, owner, justification, expiry/review cadence, scope, change history, or approval evidence are unavailable.
...
TEST-006, TEST-012, TEST-017, TEST-019. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
