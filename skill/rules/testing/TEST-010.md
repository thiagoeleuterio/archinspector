...
Evaluate whether rules based on namespace, package, folder, name, suffix, prefix, or text pattern are reliable enough for the architectural claim.
...
Apply this rule when an architecture verification relies materially on names, namespaces, folders, packages, prefixes, suffixes, annotations, or textual selectors.
...
Use Not Applicable when verification uses stronger structural evidence, naming is irrelevant, or naming is only non-decisive documentation. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether rules based on namespace, package, folder, name, suffix, prefix, or text pattern are reliable enough for the architectural claim, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Can naming-based selection support the verification without becoming brittle or misleading? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes naming selectors, matched element list, naming convention, structural corroboration, rename history, zero-match behavior, and negative cases. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether rules based on namespace, package, folder, name, suffix, prefix, or text pattern are reliable enough for the architectural claim. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: zero-match selector, where expected evidence is matched element output and the problem is a passing check over nothing; rename fragility, where expected evidence is rename history and selector logic and the problem is architecture coverage lost through naming change; text-only proof, where expected evidence is structural corroboration and the problem is treating names as direct architectural evidence.

Remediation guidance should inspect matched elements, add structural corroboration where needed, handle zero-match behavior explicitly, document stable naming conventions, and avoid banning naming when it is only supporting evidence.

Internal boundaries: TEST-009 owns whether module boundary is verified; TEST-012 owns improper rejection; TEST-013 owns missed violations. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when naming-based selectors are corroborated by structural evidence or stable conventions and evidence shows they select the intended elements without standing alone as direct proof.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and naming or textual patterns are decisive but brittle, stale, zero-match, overbroad, or unsupported by stronger evidence needed for the claim.
...
Use Warning when naming evidence is useful but partially inconsistent, weakly corroborated, or vulnerable to rename without confirmed missed or false results.
...
Use Not Applicable when verification relies on stronger structural evidence, naming is irrelevant, or naming is only non-decisive documentation.
...
Use Not Enough Evidence when selectors, matched element lists, naming convention, structural corroboration, rename history, zero-match behavior, or results are unavailable.
...
TEST-009, TEST-012, TEST-013. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
