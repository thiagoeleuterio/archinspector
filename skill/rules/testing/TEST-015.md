...
Architecture test failure diagnostics
...
Evaluate whether a failure identifies the violated constraint, involved elements, context, and necessary action sufficiently.
...
Apply this rule when an architectural verification can fail and produce messages, logs, reports, diffs, dashboards, or review output.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether a failure identifies the violated constraint, involved elements, context, and necessary action sufficiently, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Can a reviewer act on the failure without reverse-engineering the entire verification? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes failure messages, output reports, violated rule names, source and target elements, matched dependencies, context, logs, and remediation notes. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether a failure identifies the violated constraint, involved elements, context, and necessary action sufficiently. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: generic failure message, where expected evidence is output and violated rule and the problem is no actionable context; missing involved elements, where expected evidence is source/target or matched item data and the problem is reverse-engineering required; discarded failure output, where expected evidence is logs or reports and the problem is unavailable diagnostic result.

Remediation guidance should include the violated constraint, involved elements, matched dependency or scope context, and concise repair direction where useful; retain failure output; and avoid requiring perfect diagnostics for low-risk checks.

Internal boundaries: TEST-018 owns execution context; this rule owns the usefulness of failure output once a failure is produced. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when failure output gives enough information to identify the violated architectural constraint, involved elements, relevant context, and proportionate next action for the reviewed risk.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and failure output is generic, missing involved elements, missing the violated rule, discarded, misleading, or not actionable for the architectural issue.
...
Use Warning when diagnostics are partial, terse, noisy, or require moderate investigation but still allow reasonable action for the risk level.
...
Use Not Applicable when the mechanism is advisory only, has no failure mode, or diagnostics are outside reviewed scope and risk.
...
Use Not Enough Evidence when failure messages, reports, violated rule names, involved elements, logs, result retention, or remediation context are unavailable.
