...
Architecture test behavioral relevance
...
Evaluate whether the verification protects real behavior, risk, or architectural quality instead of superficial structure without impact.
...
Apply this rule when a verification claims architectural value and its assertion, metric, review step, or control can be related to behavior, risk, quality, or change impact.
...
Use Not Applicable when the check is intentionally cosmetic, local, exploratory, or not represented as architectural validation. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether the verification protects real behavior, risk, or architectural quality instead of superficial structure without impact, within the reviewed scope and without relying on nominal signals alone.
...
Primary evidence includes decision risk, protected behavior or quality, assertion logic, affected flow, failure consequence, documentation, and historical failures. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether the verification protects real behavior, risk, or architectural quality instead of superficial structure without impact. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: superficial structure check, where expected evidence is protected risk or quality and the problem is shape without consequence; metric without consequence, where expected evidence is interpretation and impact and the problem is measurement detached from architectural value; architectural label on cosmetic rule, where expected evidence is rule objective and the problem is style presented as architecture.

Remediation guidance should connect the verification to the risk or quality it protects, downgrade or rename cosmetic checks, document limitations, and avoid requiring runtime behavior proof for structural checks that have a clear architectural consequence.

Internal boundaries: TEST-001 owns fitness-function definition; TEST-012 and TEST-013 own systematic false positive and false negative control. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when evidence connects the verification to a real architectural consequence, such as protected behavior, quality attribute, risk, change impact, or failure consequence, rather than only superficial shape.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and the verification is presented as architectural but checks only cosmetic, appearance-only, or irrelevant structure without protecting the declared risk or quality.
...
Use Warning when architectural relevance is plausible but indirect, underexplained, low-impact, or carried mostly by documentation without enough evidence for failure.
...
Use Not Enough Evidence when the decision risk, protected behavior, quality attribute, assertion logic, affected flow, failure consequence, or historical evidence is unavailable.
...
TEST-001, TEST-002, TEST-012, TEST-013, TEST-019. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
