...
Evaluate whether later changes violating an accepted architectural state can be identified.
...
Apply this rule when an architecture verification protects an accepted state, baseline, historical result, governed exception set, or recurring architectural constraint.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether later changes violating an accepted architectural state can be identified, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Can the verification detect architectural regressions after a known acceptable or governed state? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes baseline comparison, historical runs, accepted state, failing regression case, changed dependency graph, suppression behavior, pipeline or scheduled execution, and review records. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether later changes violating an accepted architectural state can be identified. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: auto-updated baseline, where expected evidence is baseline change behavior and the problem is silent acceptance; unreviewed architectural change, where expected evidence is history and comparison and the problem is no regression signal; suppression masks regression, where expected evidence is suppression behavior and new violation evidence and the problem is hidden drift.

Remediation guidance should define the accepted state, compare later results against it, govern baseline changes, retain enough history, and route execution-timing or exception-lifecycle defects to adjacent Rules when primary.

Internal boundaries: TEST-002 owns traceability; TEST-016 owns exception governance; TEST-018 owns execution timing; TEST-019 owns maintainability. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when evidence shows the verification compares against an accepted or governed state and can identify relevant architectural regressions after change within the reviewed scope.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and new violations are silently accepted because baseline comparison is absent, auto-updated, unreviewed, masked by suppression, never executed, or never compared.
...
Use Warning when regression detection exists but history, comparison behavior, accepted-state definition, cadence, or governance is incomplete without confirmed silent regression.
...
Use Not Applicable when no accepted state exists, the decision is one-time, baseline comparison is unnecessary, or periodic/manual detection is adequate for low risk.
...
Use Not Enough Evidence when accepted state, baseline comparison, historical runs, changed graph, failing regression case, suppression behavior, execution result, or review records are unavailable.
...
TEST-002, TEST-013, TEST-016, TEST-018, TEST-019. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
