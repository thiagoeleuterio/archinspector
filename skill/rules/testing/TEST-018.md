...
Architecture test pipeline execution
...
Evaluate whether verification runs at a moment and context proportionate to risk and feedback needs.
...
Apply this rule when architectural checks are expected to run locally, in review, in CI, release, scheduled validation, operations, or another delivery-control point.
...
Use Not Applicable when execution is intentionally manual, one-time, low-risk, external, or unnecessary for the reviewed decision. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether verification runs at a moment and context proportionate to risk and feedback needs, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Is the verification executed where and when its architectural risk can be caught usefully? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes CI configuration, local scripts, release gates, scheduled jobs, execution logs, review process, result retention, failure handling, and documented cadence. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether verification runs at a moment and context proportionate to risk and feedback needs. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: never-run check, where expected evidence is execution logs or scripts and the problem is dormant verification; optional job ignored, where expected evidence is delivery configuration and history and the problem is no meaningful feedback point; discarded result, where expected evidence is result retention and failure handling and the problem is architectural signal lost.

Remediation guidance should place execution at a proportionate feedback point, retain and act on results, document manual cadence when adequate, and avoid requiring CI, every-commit execution, or pipeline gates universally.

Internal boundaries: TEST-014 owns deterministic results; TEST-015 owns diagnostics; TEST-017 owns regression detection; TEST-019 owns maintenance of execution setup. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when relevant architecture verification is executed at a local, review, CI, release, scheduled, operational, or manual control point that is proportionate to the architectural risk and feedback need.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and a relevant verification exists but is not run, is optional and ignored, runs too late for the risk, discards results, or is local-only despite high recurring risk.
...
Use Warning when execution is plausible but cadence, logs, result retention, failure handling, or delivery-flow placement is incomplete or weak without confirmed missed regression.
...
Use Not Applicable when execution is intentionally manual, one-time, low-risk, external, unnecessary for the reviewed decision, or outside reviewed scope.
...
Use Not Enough Evidence when CI configuration, local scripts, release gates, scheduled jobs, execution logs, review process, result retention, failure handling, or cadence evidence is unavailable.
...
TEST-014, TEST-015, TEST-017, TEST-019. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
