...
Architecture test false-negative control
...
Evaluate whether the verification avoids allowing relevant violations through gaps, fragile filters, empty selections, or superficial evidence.
...
Apply this rule when a verification claims to detect architectural violations, drift, dependency problems, boundary bypasses, or constraint breaches.
...
Use Not Applicable when the mechanism is informational only, risk is deliberately out of scope, or violation detection is handled elsewhere. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether the verification avoids allowing relevant violations through gaps, fragile filters, empty selections, or superficial evidence, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Does the verification actually fail when a relevant architectural violation exists? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes negative cases, seeded violations, selector results, assertion strength, empty-scope behavior, exclusions, baseline behavior, and execution result. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether the verification avoids allowing relevant violations through gaps, fragile filters, empty selections, or superficial evidence. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: green test with empty scope, where expected evidence is selector result and the problem is no relevant elements; always-true assertion, where expected evidence is assertion logic and the problem is no effective failure path; excessive exclusion, where expected evidence is exclusions and violation examples and the problem is relevant violations allowed through.

Remediation guidance should make assertions capable of failing, expose empty selections, add representative negative checks where useful, narrow exclusions or baselines, and avoid demanding exhaustive detection beyond stated risk and scope.

Internal boundaries: TEST-012 owns improper rejection; TEST-004 owns scope definition; TEST-010 owns naming fragility; TEST-011 owns relevance. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when evidence shows the verification can fail for relevant violations within scope, including representative negative cases, effective assertions, non-empty selection, and controlled exclusions or baselines.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and relevant violations pass because of empty selection, always-true assertions, broad exclusions, incomplete filters, superficial evidence, stale baselines, or missed indirect paths.
...
Use Warning when missed-violation risk is plausible but limited, such as partial negative cases, fragile filters, narrow coverage, or unproven baseline risk.
...
Use Not Applicable when the mechanism is informational only, violation detection is deliberately out of scope, or another proportionate control owns detection.
...
Use Not Enough Evidence when negative cases, selector results, assertion strength, empty-scope behavior, exclusions, baseline behavior, execution result, or reviewed scope are unavailable.
...
TEST-004, TEST-007, TEST-010, TEST-011, TEST-012, TEST-017. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
