...
Evaluate whether explicitly prohibited and architecturally relevant dependencies can be detected reliably when risk justifies it.
...
Apply this rule when a decision, rule, policy, or verification identifies dependencies that must not exist and the risk justifies repeatable detection.
...
Use Not Applicable when no prohibited dependency exists, the risk is negligible, manual review is adequate, or prohibition is outside reviewed scope. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether explicitly prohibited and architecturally relevant dependencies can be detected reliably when risk justifies it, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Can the verification expose the forbidden dependency paths that matter for the architectural risk? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes forbidden dependency list, detection rule, selectors, dependency graph, seeded failing case, exclusions, suppressions, and execution result. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether explicitly prohibited and architecturally relevant dependencies can be detected reliably when risk justifies it. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: documented prohibition without detection, where expected evidence is a rule or repeatable process and the problem is convention-only control; exclusion hiding forbidden paths, where expected evidence is exclusions and graph evidence and the problem is broad omission; alias or indirect reference escape, where expected evidence is dependency graph and selector behavior and the problem is missed forbidden access.

Remediation guidance should define the prohibited relationship, align detection with real dependency evidence, narrow exclusions, add a seeded or representative failing case where useful, or preserve a repeatable manual control when automation is not proportionate.

Internal boundaries: TEST-005 owns generic dependency-rule coherence; TEST-009 owns modular boundary representation; TEST-016 owns governed exceptions. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when prohibited dependency paths are explicitly identified, the detection mechanism can observe the relevant graph or reviewed material, and evidence shows it would expose the forbidden dependency within the reviewed scope.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and a relevant prohibited dependency is not detected because the rule is absent, selector logic misses it, exclusions hide it, or the mechanism relies only on convention.
...
Use Warning when forbidden dependency detection exists but is partial, brittle, manual without enough repeatability, or limited to lower-risk paths.
...
Use Not Applicable when no prohibited dependency exists, the architectural risk is negligible, manual review is adequate, or the prohibition is outside reviewed scope.
...
Use Not Enough Evidence when the forbidden list, detection logic, graph, selectors, exclusions, execution result, or manual review evidence is unavailable or insufficient.
...
TEST-005, TEST-009, TEST-016. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
