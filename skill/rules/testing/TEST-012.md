...
Architecture test false-positive control
...
Evaluate whether the verification avoids rejecting legitimate implementations that preserve architectural intent.
...
Apply this rule when a verification can reject implementations, dependencies, modules, naming forms, exceptions, or equivalent architectural solutions.
...
Use Not Applicable when the mechanism does not produce rejection, the decision intentionally forbids alternatives, or manual judgment controls acceptance. Legitimate absence may occur in small systems, prototypes, temporary systems, low-risk decisions, partial review scopes, externally governed controls, deliberately manual validation contexts, or properties that are not reasonably repeatable or automatable.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether the verification avoids rejecting legitimate implementations that preserve architectural intent, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Does the verification avoid failing acceptable architectural alternatives? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes failing cases, assertion logic, selector exclusions, equivalent patterns, exception rationale, reviewed alternatives, and failure history. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether the verification avoids rejecting legitimate implementations that preserve architectural intent. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: legitimate alternative rejected, where expected evidence is reviewed alternative and intent and the problem is false failure; brittle selector failure, where expected evidence is selector logic and failing case and the problem is irrelevant rejection; exception rationale ignored, where expected evidence is governed exception and assertion behavior and the problem is valid deviation treated as violation.

Remediation guidance should refine assertions or selectors, encode or document legitimate alternatives, distinguish exceptions from broad allowances, and keep missed-violation analysis in TEST-013 rather than weakening protection indiscriminately.

Internal boundaries: TEST-013 owns violations that pass; TEST-007 owns allow precision; TEST-010 owns naming reliability; TEST-011 owns relevance. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when rejection behavior is precise enough to distinguish actual architectural violations from legitimate alternatives, governed exceptions, or equivalent designs that preserve intent.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and the verification rejects legitimate implementations, valid extension points, governed exceptions, or equivalent architectures without architectural justification.
...
Use Warning when rejection risk is plausible but limited, such as brittle selectors, incomplete exception rationale, or narrow false-failure history without confirmed legitimate rejection.
...
Use Not Applicable when the mechanism does not reject implementations, the decision intentionally forbids alternatives, or manual judgment controls acceptance.
...
Use Not Enough Evidence when failing cases, assertion logic, selector exclusions, equivalent alternatives, exception rationale, reviewed alternatives, or failure history are unavailable.
...
TEST-007, TEST-010, TEST-011, TEST-013, TEST-016. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
