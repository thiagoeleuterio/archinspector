...
Evaluate whether boundaries between modules, components, or architectural units are verified according to the declared architecture.
...
Apply this rule when the reviewed material contains module, component, bounded context, package, namespace, project, service, or architectural-unit boundary checks.
...
An architectural verification mechanism must provide sufficient, traceable, and proportionate evidence of whether boundaries between modules, components, or architectural units are verified according to the declared architecture, within the reviewed scope and without relying on nominal signals alone.
...
The architectural question is: Does the verification represent and check the declared module or component boundary? This rule does not require universal architecture tests, automation, a specific tool, CI/CD, execution on every commit, a numeric threshold, a percentage, zero exceptions, zero baselines, zero suppressions, or a particular language or framework.
...
Primary evidence includes module map, boundary rule, dependency checks, public contracts, owned namespaces, API use, selectors, bypass paths, and documentation. Stronger evidence includes executable or repeatable implementation, real selection of elements, effective assertions, configuration, execution results, positive and negative cases, tests of the mechanism, failure history, baselines, suppressions, ADRs, and documentation tied to the reviewed system.
...
Then evaluate whether the verification satisfies this rule's exclusive responsibility: whether boundaries between modules, components, or architectural units are verified according to the declared architecture. Interpret evidence conservatively and separate the verification mechanism from the underlying architecture condition. A violation of an architectural boundary, dependency rule, domain model, layer, event, SOLID principle, or solution decision belongs to that owning catalog unless the conclusion is about the adequacy of the validation mechanism.

Common finding shapes include: folder-only module identification, where expected evidence is module map and structural corroboration and the problem is nominal boundary mapping; boundary bypass omitted, where expected evidence is API/dependency paths and the problem is unverified access around the contract; new component outside map, where expected evidence is current modules and selectors and the problem is stale boundary coverage.

Remediation guidance should align the module map with the declared architecture, verify public contracts or dependency paths proportionately, document logical boundaries, and avoid requiring a specific packaging style such as assemblies or folders.

Internal boundaries: TEST-004 owns scope adequacy; TEST-005/006/008 own dependency-specific checks; TEST-010 owns naming selector reliability. Shared evidence may be used by neighboring rules, but this rule's conclusion remains independent and no neighboring rule is a procedural prerequisite.
...
Use Pass when the verification represents declared modules, components, or architectural units and checks their boundaries through evidence such as dependency rules, public contracts, owned namespaces, APIs, or repeatable manual review.
...
Use Fail only when applicability is confirmed, evidence is sufficient, and the verification misses boundary-crossing paths, uses the wrong module map, ignores public contracts, or cannot distinguish allowed interaction from boundary bypass.
...
Use Warning when boundary representation is plausible but module mapping, selectors, contracts, or bypass coverage are incomplete or fragile.
...
Use Not Applicable when no modular boundary is claimed, boundary verification is outside scope, or a logical boundary is intentionally validated manually with proportionate control.
...
Use Not Enough Evidence when module maps, boundary rules, selectors, public contracts, dependency/API evidence, documentation, or results are unavailable or insufficient.
...
TEST-004, TEST-005, TEST-006, TEST-008, TEST-010. These are related Architecture Testing rules with adjacent responsibilities, not procedural prerequisites.
